...
_GENERIC_REPORTS_DB_ERROR = "Erro ao consultar o histórico de relatórios. Tente de novo em instantes."
...
def _require_report_access(report: dict, user: dict) -> None:
    ...
    if user["login"].lower() in MANAGEMENT_PANEL_LOGINS:
        return
...
def _get_report_or_404(report_id: str) -> dict:
    try:
        ...
        raise _log_and_generic_error(e, generic_message=_GENERIC_REPORTS_DB_ERROR)
    if report is None:
...
async def list_reports_endpoint(
    ...
    is_manager = _user["login"].lower() in MANAGEMENT_PANEL_LOGINS
    ...
    effective_created_by = created_by if is_manager else _user["login"]
    try:
...
@app.get("/reports/{report_id}/versions/{version_id}")
...
@app.get("/reports/{report_id}/generations")
...
@app.get("/reports/{report_id}/artifacts")
